...
Capricorn::Capricorn(EntityFaction faction)
	: Entity(EntityType::CAPRICORN, faction)
{
	m_physicsRadius = 0.25f;
	m_shootCoolDownSeconds = g_gameConfigBlackboard.GetValue("capricornShootCooldownSeconds", m_shootCoolDownSeconds);
	m_driveSpeed = g_gameConfigBlackboard.GetValue("capricornDriveSpeed", m_driveSpeed);
	m_turnSpeed = g_gameConfigBlackboard.GetValue("capricornTurnRate", m_turnSpeed);
	m_driveAperture = g_gameConfigBlackboard.GetValue("capricornDriveAperture", m_driveAperture);
	m_shootAperture = g_gameConfigBlackboard.GetValue("capricornShootAperture", m_shootAperture);
	m_maxHealth = g_gameConfigBlackboard.GetValue("capricornHealth", m_maxHealth);
	...
	m_baseTexture = g_theRenderer->CreateOrGetTextureFromFile("Data/Images/EnemyTank3.png");
}
...
void Capricorn::Execute(float deltaSeconds)
{
	// TODO make the a component to read map info and give target position to the enemy
	Vec2 targetPos;
...
void Capricorn::Shoot()
{
	if (m_shootCoolDownTimer <= 0.f)
	{
		Vec2 muzzlePos = m_position + Vec2::MakeFromPolarDegrees(m_orientationDegrees, 0.3f);
		m_map->SpawnNewEntity(EntityType::MISSILE, EntityFaction::EVIL, muzzlePos, m_orientationDegrees);
